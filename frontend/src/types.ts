 ...
 */
export interface FapAnalysisJson {
  date?: {
    date?: string;
    start?: string;
    end?: string;
 ...
 */
export interface FapAverageJson {
  overall?: {
    distance_km?: number;
    duration?: {
      overall_sec?: number;
...
/**
 * Response model for endpoints issuing access tokens (login/refresh).
 */
 ...
 */
export interface UploadAnalysisResponseDto {
  ids: FapAnalysis["id"][];
}
...
export interface AnalysisDetailDto {
  ...
  status: AnalysisStatus;
  message: string;
  logDate: string | null; // ISO string from API
  fapRegen: boolean;
  distance: number | null;
  analysis: FapAnalysisJson | null;
  version: string;
...
/**
 * DTO for 409 Conflict on upload (duplicate content) with existing analysis ID.
 */
export interface ConflictExistingAnalysisDto {
  id: FapAnalysis["id"];
}
