<template>
  <div class="container-fluid py-4">
    ...
    <div v-else-if="averageData" class="row">
      <div class="col-12 mb-4">
        <div class="card">
          <div class="card-header">
            <h5 class="card-title mb-0">Summary</h5>
          </div>
          <div v-if="averageData.status.toLowerCase() !== 'success'" class="card-body">
            <h6>Status: <span :class="statusClass">{{ averageData.status }}</span></h6>
            <div v-if="averageData.message" class="alert alert-danger mt-3">
              {{ averageData.message }}
            </div>
          </div>
        </div>
      ...
      <template v-if="averageData.average">
        <div v-for="(section, sectionKey) in (averageData.average as AverageData)" :key="sectionKey" class="col-md-6 col-lg-4 mb-4">
          <div class="card h-100">
            <div class="card-header">
              <h5 class="card-title mb-0">{{ formatSectionTitle(sectionKey) }}</h5>
            </div>
            <div class="card-body">
              <template v-for="(value, key) in (section as AverageData)" :key="key">
                <div v-if="value !== null" class="mb-2">
                  <template v-if="typeof value === 'object' && value !== null">
                    <h6 class="mb-2">{{ formatKey(key) }}</h6>
                    <template v-for="(subValue, subKey) in (value as AverageData)" :key="subKey">
